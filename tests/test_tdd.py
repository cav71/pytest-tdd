...
def _project_simple(mktree):
    ...
    tst = workdir / "tests" / "package" / "subpackage" / "test_mode.py"
    tst.write_text(
        textwrap.dedent(
            """
    import pytest
...
def test_run_simple(mktree):
    workdir = _project_simple(mktree)
    candidates = [
        workdir / "tests" / "package" / "subpackage" / "test_mode.py",
    ]

    ret, out = tdd.run(workdir, "package", candidates, workdir / "src")
    ...
    assert (
        coverage["files"][str(Path("src/package/subpackage/mod.py"))]["summary"][
            "percent_covered"
        ]
        == 75.0
    )
...
def test_process(resolver):
    ...
    assert (
        tdd.process(Path("a/b/file-name"), result)
        == "file-name run 4 tests with 1 failures and 0 errors, covered 3 lines out "
        "of 4 (75.0%, missing=1 lines)"
    )
